...
**A:** A function that accepts **any number of arguments** of the same type.  
**Syntax:**  
```go
func foo(values ...int) { }  // 'values' becomes []int inside the function
```

---

#### **2. How does Go implement variadic functions?**
**A:** Go converts arguments into a **slice**.  
...
**A:** Not directly, but use `interface{}` (loses type safety):  
```go
func bar(vals ...interface{}) {
    for _, v := range vals {
        fmt.Printf("%T\n", v)  // Prints type of each arg
    }
}
bar(1, "hello", true)  // Works, but risky
```
...
**A:** The parameter becomes an **empty slice** (`len=0`). Always check:  
```go
...
### **Interview Ready? Test Yourself:**
1. How would you sum any number of integers?  
   **A:** `func Sum(nums ...int) int`  
2. How to pass `[]string{"a","b"}` to `func Join(...string)`?  
   **A:** `Join(slice...)`  
3. What’s inside `values` in `func f(values ...int)` if called as `f()`?  
   **A:** Empty slice (`[]int{}`).
...
Variadic functions in Go accept **a variable number of arguments** of the same type. They're declared using `...` before the type in the function signature.
...
### Q1: What happens when no arguments are passed?

**Answer**: The parameter becomes an empty slice (`[]T{}`), not nil. Always check length:
...
### Q2: Can we have multiple variadic parameters?

**Answer**: No. Go enforces:
- Exactly one variadic parameter
- Must be the final parameter

### Q3: How to achieve type flexibility?

**Answer**: Use `...interface{}` but with type safety trade-offs:
